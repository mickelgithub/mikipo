...
class NotificationRemoteDatasourceImpl
  ...
  static const String _FLUTTER_NOTIFICATION_CLICK =
  ...
  static final _logger =
      getLogger((NotificationRemoteDatasourceImpl).toString());
  ...
  NotificationRemoteDatasourceImpl(this._firebaseMessaging, this._httpClient,
      this._messagingServerUrl, this._messagingServerKey);
  ...
  Future<void> sendNotification(Notification notification) async {
    ...
    _logger.d(data);
    final response = await _httpClient.post(_messagingServerUrl,
        headers: {
          _AUTHORIZATION_HEADER: 'key=$_messagingServerKey',
        ...
        body: jsonEncode(<String, dynamic>{
          ...
          _NOTIFICATION_DATA_KEY: notification.extraData != null &&
    ...
    if (response.statusCode == 200) {
      ...
      if (sucess > 0 && error == 0) {
        _logger.d(
            'ya hemos enviado correctamente la notificacion a ${notification.to.email}');
      } else {
        final error = (bodyMap['results'] as List).first['error'];
        _logger.d(
            'Ha ocurrido este error al enviar la notificacion a ${notification.to.email} ---> $error');
        //TODO
        throw CustomException(CustomCause.notifRecipientIncorrect);
      }
    } else {
      //TODO
      throw CustomException(CustomCause.errorNotControlled);
    }
  }
}
