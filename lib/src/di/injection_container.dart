...
Future<void> init() async {
  ...
  //datasources
  serviceLocator.registerSingleton<IAuthRemoteDatasource>(AuthRemoteDatasourceImpl(serviceLocator()));
  serviceLocator.registerSingleton<IUserLocalStorageDataSource>(UserLocalStorageDataSourceImpl(serviceLocator()));
  serviceLocator.registerSingleton<IUserRemoteStorageDataSource>(UserRemoteStorageDataSourceImpl(serviceLocator()));
  serviceLocator.registerSingleton<IOrganizationRemoteDatasource>(OrganizationRemoteDatasourceImpl(serviceLocator()));
  serviceLocator.registerSingleton<IAvatarRemoteDatasource>(AvatarRemoteDatasourceImpl(serviceLocator()));
  serviceLocator.registerFactory<IAvatarLocalDatasource>(() => AvatarLocalDatasourceImpl(serviceLocator()));
  serviceLocator.registerFactory<IChefRemoteStorageDataSource>(() => ChefRemoteStorageDataSourceImpl(serviceLocator()));
  serviceLocator.registerFactory<INotificationRemoteDatasource>(() => NotificationRemoteDatasourceImpl(serviceLocator(),serviceLocator(),
      'https://fcm.googleapis.com/fcm/send', 'AAAATqGctIE:APA91bEPQwZPzIToKA5BbL3g0djGEL7EFxEe_ghwULs4iho-SJxtB_pNQIJb7s0oGyFwykqu7vdS70-EC6iZEpJCDvgxazyH9w_3Q36Om_vEQkVvKXpo20d2aCLh13YQIAQZ8XPdnz9D'));
  serviceLocator.registerFactory<ITeamRemoteDatasource>(() => TeamRemoteDatasourceImpl(serviceLocator()));



  //repositories
  serviceLocator.registerSingleton<IAuthRepository>(AuthRepositoryImpl(serviceLocator(), serviceLocator(), serviceLocator(), serviceLocator(), serviceLocator(), serviceLocator(), serviceLocator()));
  serviceLocator.registerSingleton<IOrganizationRepository>(OrganizationRepositoryImpl(serviceLocator()));
  serviceLocator.registerFactory<IAvatarRepository>(() => AvatarRepositoryImpl(serviceLocator(), serviceLocator()));
  serviceLocator.registerFactory<IUserRepository>(() => UserRepositoryImpl(serviceLocator(), serviceLocator()));
  serviceLocator.registerFactory<IChefRepository>(() => ChefRepositoryImpl(serviceLocator()));
  serviceLocator.registerFactory<INotificationRepository>(() => NotificationRepositoryImpl(serviceLocator()));
  serviceLocator.registerFactory<ITeamRepository>(() => TeamRepositoryImpl(serviceLocator(), serviceLocator(), serviceLocator(), serviceLocator()));

  //use cases
  serviceLocator.registerFactory(() => GetUserStreamUseCase(serviceLocator()));
  serviceLocator.registerFactory(() => GetOrganizationInfoUseCase(serviceLocator()));
  serviceLocator.registerFactory(() => CropImageUseCase(serviceLocator()));
  serviceLocator.registerFactory(() => RegisterUserUseCase(serviceLocator(), serviceLocator(), serviceLocator(), serviceLocator(), serviceLocator()));
  serviceLocator.registerFactory(() => PickImageUseCase(serviceLocator()));
  serviceLocator.registerFactory(() => DeleteAvatarFromCacheUserCase(serviceLocator()));
  serviceLocator.registerFactory(() => LoginUserUseCase(serviceLocator(), serviceLocator(), serviceLocator()));
  serviceLocator.registerFactory(() => GetTeamMembersUseCase(serviceLocator()));
  serviceLocator.registerFactory(() => AcceptDenyMemberUseCase(serviceLocator()));
  serviceLocator.registerFactory(() => HandleAcceptDenyMemberNotificationUseCase(serviceLocator()));



  //viewmodels
  serviceLocator.registerFactory(() => AuthBuilderWidgetViewModel(serviceLocator()));
  serviceLocator.registerFactory(() => LoginViewModel(serviceLocator(), serviceLocator(),
      serviceLocator(), serviceLocator(), serviceLocator(), serviceLocator()));
  serviceLocator.registerFactory(() => HomeViewModel());
  serviceLocator.registerFactory(() => TeamViewModel(serviceLocator(), serviceLocator()));
